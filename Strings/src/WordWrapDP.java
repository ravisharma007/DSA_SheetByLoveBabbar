...
class Solution
{
    public int solveWordWrap (int[] nums, int k)
    {
        // Code here
        int n=nums.length;
        int[][] extras=new int[n+1][n+1]; //extra space
        int[][] lc=new int[n+1][n+1]; //line Cost

        int[] oc=new int[n+1]; // optimal cost
        ...
        for(int j=1;j<=n;j++)
        {
            oc[j]=Integer.MAX_VALUE;
            for(int i=1; i<=j; i++)
            {
                if(oc[i-1] != Integer.MAX_VALUE && lc[i][j] != Integer.MAX_VALUE &&
                        oc[i-1] + lc[i][j] < oc[j] )
                {
                    oc[j] = oc[i-1] + lc[i][j];
                }
